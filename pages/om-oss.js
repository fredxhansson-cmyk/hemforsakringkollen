  ...
  return (
    <>
      ...
      <nav style={{ background:'#fff', borderBottom:'1px solid #e2e8f0', padding:'0 20px', height:60, display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:100, fontFamily:'Inter,sans-serif' }}>
        <Link href="/" style={{ fontWeight:800, fontSize:18, color:pc, textDecoration:'none' }}>HEMkollen</Link>
      ...
      <main style={{ maxWidth:760, margin:'0 auto', padding:'56px 20px', lineHeight:1.8, color:'#374151', fontFamily:'Inter,sans-serif' }}>
        <h1 style={{ fontSize:38, fontWeight:800, marginBottom:8, color:'#0f172a' }}>Om HEMkollen</h1>
        <p style={{ fontSize:17, color:'#64748b', marginBottom:40 }}>HEMkollen drivs av Qubeit — ett svenskt bolag som bygger oberoende jämförelsetjänster för svenska konsumenter.</p>
        <h2 style={{ fontSize:22, fontWeight:700, marginBottom:12, color:'#0f172a' }}>Vad vi gör</h2>
        <p style={{ marginBottom:20, fontSize:15 }}>HEMkollen samlar och presenterar information om leverantörer inom försäkring. Vi rangordnar och betygsätter baserat på pris, villkor, kundservice och användarrecensioner. Vår trafik kommer från organisk söktrafik via Google och Bing.</p>
